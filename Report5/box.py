...
def main():
    ...
    for ksize in range(kmin, kmax+1, 2):
        ...
        print('filter2D:{:6.2f}'.format((time.time()-s)*1000), end='ms\t')
        kernel = np.ones(ksize, np.float64) / ksize
        s = time.time()
        res = cv2.filter2D(img, cv2.CV_8UC3, kernel, kernel)
        print('sepFilter:{:6.2f}'.format((time.time()-s)*1000), end='ms\t')
        s = time.time()
        res = cv2.boxFilter(img, cv2.CV_8UC3, (ksize, ksize))
        print('boxFilter:{:6.2f}'.format((time.time()-s)*1000), end='ms\t')
        s = time.time()
        res = myboxFilter(img, (ksize, ksize))
        print('myBoxFilter:{:6.2f}'.format((time.time()-s)*1000), end='ms\t')
        print('')
